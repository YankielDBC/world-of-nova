...
export function formatPercentFromFraction(value, digits = 1) {
    const pct = value * 100;
...
export function formatSignedFlat(value, digits = 1) {
    return `${value >= 0 ? '+' : ''}${formatNumber(value, digits)}`;
}
export function formatActionLabel(action, lang) {
    if (action === 'chop')
...
export function formatEventLabel(event, lang) {
...
export function formatCondition(condition, lang) {
    const pieces = [];
...
export function collectEffectTokens(effectSet, scale, lang) {
    if (!effectSet)
        return [];
    const tokens = [];
...
export function formatTokenRows(tokens) {
    const rows = [];
...
export function buildRankSummaryLines(def, rank, lang) {
    const lines = [];
...
export function getPreviewTokens(def, lang) {
    if (def.passiveEffectsPerRank) {
...
export function collectThemesFromEffectSet(effectSet, out) {
    if (!effectSet)
        return;
    const mods = effectSet.combatModifiers;
...
export function collectThemes(def) {
...
export function evaluatePairSynergy(base, candidate, lang) {
...
export function isEquipped(state, skillKey) {
